...
async function getApplicationForConditionalWrite(id) {
  ensureBlobIsConfigured()
  const pathname = recordPath(id)
  ...
  for (let attempt = 0; attempt < 3; attempt += 1) {
    ...
    const versionStayedStable = comparableEtag(before.etag) === comparableEtag(after.etag)
    const contentMatchesVersion = comparableEtag(result.blob.etag) === comparableEtag(after.etag)
    if (!versionStayedStable || !contentMatchesVersion) continue

    Object.defineProperty(application, RECORD_ETAG, { value: after.etag, writable: true })
    return application
  }
...
export async function updateApplication(id, changes = {}) {
  ...
  try {
    ...
    const latest = await getApplicationForConditionalWrite(id)
